...
RSpec.describe RelationValue, type: :model do
  ...
  describe '#draft_value[=]' do
    it 'updates associated draft posts list' do
      posts = 3.times.map{ create :post }
      relation_value.draft_value = posts
      relation_value.save!

      reloaded = RelationValue.find relation_value.id

      expect(reloaded.draft_posts).to match_array posts
    end

    it 'does not recreate DraftRelationValuePost(s)' do
      relation_value.draft_value = 3.times.map{ create :post }
      relation_value.save!

      initial_ids = relation_value.draft_relation_value_posts.pluck(:id)
      expect(initial_ids.size).to eql 3 # foolproof check

      relation_value.draft_value = relation_value.draft_value
      relation_value.save!

      expect(relation_value.draft_relation_value_posts.pluck(:id)).to match_array initial_ids
    end

    it 'defaults to nil value' do
      expect(described_class.new.draft_value).to be_nil
    end

    it 'allows nil value' do
      relation_value.draft_value = nil
      relation_value.save!

      reloaded = described_class.find relation_value.id
      expect(reloaded.posts).to be_empty
      expect(reloaded.draft_value).to be_nil
    end
  end
  ...
  example '#draft_posts returns all draft associated Post(s)' do
    posts = 3.times.map do
      post = create :post
      create :draft_relation_value_post, post: post, relation_value: relation_value
      post
    end

    expect(relation_value.draft_posts).to match_array posts
  end
end
